...
{ key: "tools", icon: " ", label: "Хэрэгсэл" },
{ key: "saved", icon: " ", label: saved.length ? `(${saved.length})` : "Хадгалсан" },
];
const SUBTOOLS = [
...
<button onClick={e => { e.stopPropagation(); toggleSave(job.id); }} style={{ background: "transparent", border: "none", fontSize: 20, cursor: "pointer", opacity: saved.includes(job.id) ? 1 : 0.3 }}> </button>
</div>
...
<div style={{ fontSize: 13 }}>Ажлын зарнаас дарж хадгалаарай</div>
</div>
...
<button onClick={() => toggleSave(selectedJob.id)} style={{ flex: 1, padding: 14, borderRadius: 12, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#fff", fontSize: 14, cursor: "pointer" }}>{saved.includes(selectedJob.id) ? " Хадгалсан" : " Хадгалах"}</button>
